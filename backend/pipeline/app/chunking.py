...
"""
Language-aware source chunker using tree-sitter with a line-based fallback.
"""
...
from app.services.languages import get_parser_for_path
from app.config import CHUNK_MAX_CHARS, CHUNK_OVERLAP

# assisted with chatgpt o1
...
def chunk_source_code(
    ...
    if parser and num_lines > 0:
        try:
            ...
            logging.warning("Tree-sitter parsing failed for %s: %s", path, exc)
            chunks = []
