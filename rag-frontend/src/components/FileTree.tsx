...
import { listRepoFiles, getRepoFile } from "../api/repos";
import { buildProgressWsUrl } from "../api/ws";
...
export function FileTree({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const files = await listRepoFiles(repoId, token);
        if (!cancelled) setAllPaths((files ?? []).sort());
      } catch (err) {
  ...
  useEffect(() => {
    ...
    const url = buildProgressWsUrl(token, repoId);
    let closed = false;
  ...
  const onSelect = useCallback(
    async (_selectedKeys: React.Key[], info: { node: EventDataNode }) => {
      ...
      try {
        const res = await getRepoFile(repoId, node.path, token || undefined);
        onSelectFile(node.path, res.content);
      } catch {
